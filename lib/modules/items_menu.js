...
module.exports = {
  ...
  searchItemMenu : function(codigo, callback){
    req.dbEntity.query(`SELECT * FROM ItemMenuIngrediente WHERE codigo_item = ${req.dbEntity.escape(codigo)};`, function(err, result){
      if(err)
        return callback(err);
      if(result.length == 0)
      ...
      callback(null, itemMenu);
    });
  },
  getItemsMenu : function(callback){
    req.dbEntity.query(`SELECT * FROM ItemMenuIngrediente`, function(err, result){
      ...
      return callback(null, itensMenu);
    });
  },
